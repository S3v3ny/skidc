...
*定义**起点**和**目标**，PentestMesh 即编排大语言模型驱动的智能体，*
...
> **命名说明：** PentestMesh 是新的对外项目名称。迁移期间，内部 Python 包和 CLI 暂时保留 `skidc` 名称，以维持兼容性。

## 什么是 PentestMesh？

PentestMesh 是一个自动化渗透测试平台，利用多个大语言模型驱动的智能体贯穿渗透测试全生命周期——
从信息收集到漏洞利用——无需手动编写逐步脚本。

你只需描述**起点**（目标 IP、URL、APK 或域名）和**成功标准**（获取 shell、拿到 flag、
发现 IDOR 等）。PentestMesh 的调度器将问题拆解为并行探索任务，在隔离的 Docker 容器中执行，
...
PentestMesh 内置 Android 控制桥接，用于移动应用安全测试。它是一个轻量级 FastAPI 服务，
...
PentestMesh 面向已授权的渗透测试 / CTF / 安全评估环境。**仅**在你拥有明确操作授权的情况下使用。
